 ...
 /* if(index == 45)printf(" here is 45\n"); */
        ...
        else
        {
            ...
            */
                ...
                */
                if(index2 >= 0)
                {
                    ...
                    */
                    ...
                    if (isCanPopReadStateI0SetInSMOBuff(obj,segId,id) == 1)
                    {
                        ret = (popReadStateI0SetInSMOBuffer(obj,segId,id) && (ret == 1));
                    }
                    else
                    {
                        index = num;
            ...
            if (ret == 0)
            {
                printf("Error(2) in mainLearningLoopInSMOBuffer, ret = 0, oldNum:%d numloop:%d,ln:%d ,file:%s\n",oldNum,num,__LINE__,__FILE__);
            }
            else
            {
                /* do nothing */
...
/*
if ((tmp3 > 18.0) && (tmp3 < 19.9))
{
printf("-XYD- index1 :%d F1val:%f ",index1,tmp3);
*/
                tmp3 = (m_F2) +(m_y1)*(m_k12)*(a1-m_alpha1)+((m_y2)*(m_k22)*(a2-m_alpha2));
/*
if ((tmp3 > 18.0) && (tmp3 < 19.9))
{
printf("-XZD- index2 :%d F2val:%f ",index2,tmp3);
}
*/
...
/*
if ((m_F2 > 18.0) && (m_F2 < 19.9))
{
printf("-XD- index :%d F2val:%f ",index,m_F2);
}
*/
...
int isCanPopReadStateI0SetInSMOBuff(struct SMOBuffer *obj,const unsigned int segId,const unsigned int id)
{
    int ret = (obj != NULL);
    //3open
    ret = isCanPopStateNxtDat(&(m_I0),segId,id);
    //1endisCanPopReadStateI0SetInSMOBuff//2end
    return ret;
}
int popReadStateI0SetInSMOBuffer(struct SMOBuffer *obj,const unsigned int segId,const unsigned int id)
{
    int ret = (obj != NULL);
    //3open
    ret = popStateNxtDat(&(m_I0),segId,id);
    if (ret == 0)
    {
        printf("Error(1) in popReadStateI0SetInSMOBuffer, ret = 0, ln:%d, fl:%s \n",__LINE__,__FILE__);
    }
    else
    {
        /* do nothing */
...
int gtNSVSampleInSmoBuff(struct SMOBuffer *obj)
{
    int ret = (obj != NULL);
    //3open
    ret = numEleRVSetNotRedunt(&(m_supportVectSet));
    //1endgtNSVSampleInSmoBuff//2end
    return ret;
}
...
int nxtDatInSMOBuffer(struct SMOBuffer *obj,int opt)
{
    ...
    else if (opt == 1)
    {
        for (index = m_currIndex;index < num;index++)
        {
            updateValInSMOBuff(obj,READ_ALPHA,index,&dTmp);
            updateValInSMOBuff(obj,READ_YCURR,index,&dTmp2);
            if ((dTmp != 0.0) && (dTmp2 != 0.0))
            {
                m_currIndex = index;
                index = num+1;
            }
