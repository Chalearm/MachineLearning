...
void makeEmptyReplaceW(replaceW *rel)
{
	...
	if(rel->oldWord != NULL)
	{
	 printOpt(__LINE__+1,"makeEmptyReplaceW","replaceWord.c",(void*)rel->oldWord,1);
		free(rel->oldWord);
		rel->oldWord = NULL;
	}
	else
	...
	if(rel->newWord != NULL)
	{
		 printOpt(__LINE__+1,"makeEmptyReplaceW","replaceWord.c",(void*)rel->newWord,1);
		free(rel->newWord);
		rel->newWord=NULL;
	}
	else
...
void setAttrb(replaceW *rel,const char* old,const char* nw,int count)
{
	...
	printOpt(__LINE__-1,"setAttrb","replaceWord.c",(void*)rel->oldWord,0);
	rel->newWord = (char*)malloc(count2*sizeof(char));
    printOpt(__LINE__-1,"setAttrb","replaceWord.c",(void*)rel->newWord,0);
...
char* replaceNWord(const char *buffer,const char *old,const char *nw,int count)
{
    ...
    printOpt(__LINE__-1,"replaceNWord","replaceWord.c",(void*)newBuff,0);
	if(newBuff == NULL)
	{
		return NULL;
	}
