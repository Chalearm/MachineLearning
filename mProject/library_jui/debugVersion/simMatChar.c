...
vector_char 	    new_vector_char(usint size)
{
	...
	if(size >0)
	{
        ...
        printOpt(__LINE__-1,"new_vector_char","simMatChar.c",(void*)data.data,0);		
		if(data.data !=0) 
		{
			data.lenght = size;
		}	
...
matrix_char	     	new_matrix_char(usint row,usint col)
{
	...
	printOpt(__LINE__-1,"new_matrix_char","simMatChar.c",(void*)data.data,0);	
	...
	printOpt(__LINE__-1,"new_matrix_char","simMatChar.c",(void*)data.data[i],0);	
...
void delete_vector_char(vector_char *rhs)
{
	...
	{	printOpt(__LINE__+1,"delete_vector_char","simMatChar.c",(void*)rhs->data,1);	
		free(rhs->data);
		rhs->data=0;
		rhs->lenght=0;
	}
...
void delete_matrix_char(matrix_char *rhs)
{
	...
	if(rhs->data !=0)
	{
		for(i=0;i<rhs->row;i++)
		{
		printOpt(__LINE__+1,"delete_matrix_char","simMatChar.c",(void*)rhs->data[i],1);	
			free(rhs->data[i]);
			rhs->data[i] =0;
		}	
		printOpt(__LINE__+1,"delete_matrix_char","simMatChar.c",(void*)rhs->data,1);	
		free(rhs->data);
		rhs->data=0;
		rhs->row =0;
		rhs->col =0;
...
element determinant(matrix_char *a)
{
	...
	matrix_char b = new_matrix_char(0,0);
	if(a->row <1)
	{
		return 	det;
	}
	...
	{
	   b = new_matrix_char(a->row-1,a->row-1);
...
void matrixInverse(matrix_char *a,matrix_char *c)
{
	...
	colFactorMatrix(a,c);
 	scalarmultiply_m_char(c,&detInverse,c); 
}
