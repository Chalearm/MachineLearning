...
void initial_vector_int(vector_int* rhs,SetValue set)
{
    ...
    switch(set.set)
    {
        ...
        break;
        case 3:
            del = (int)((set.max-set.min)/(int)(rhs->lenght));
            set.set=2;
...
void initial_matrix_int(matrix_int* rhs,SetValue set)
{
    ...
    switch(set.set)
    {
        ...
        case 2:
            for(i=0;i<rhs->row;i++)
               for(j=0;j<rhs->col;j++)
                rhs->data[i][j] =(int)set.max*((j+i)%2==0)+(int)set.min*((j+i)%2==1);
        break;
        case 3:
            del = (int)((set.max-set.min)/(int)(rhs->row*rhs->col));
            set.set=2;
...
int determinantInt(matrix_int *a)
{
    ...
    matrix_int b = new_matrix_int(0,0);
    if(a->row <1)
    {
        return  det;
    }
    ...
    {
       b = new_matrix_int(a->row-1,a->row-1);
       for(index0 = 0 ;index0 < a->row;index0++)
       {
        ...
        det += (((index0%2)==0)?1.0:-1.0)*a->data[index0][0]*determinantInt(&b);
        }
...
{
    int detInverse = 1.0/determinantInt(a);
    colFactorMatrixInt(a,c);
    scalarmultiply_m_int(c,&detInverse,c); 
}
